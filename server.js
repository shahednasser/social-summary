  ...
  return request.get(request_data.url).then(function(response) {
    ...
    for (status of statuses) {
      ...
      let p = {
        caption: status.text,
        img:
          status.entities.media && status.entities.media.length
      ...
      for (post of response.data.items) {
        ...
        let p = {
          caption: metatags.hasOwnProperty('og:description')
            ...
            : '',
          img: metatags.hasOwnProperty('og:image') ? metatags['og:image'] : '',
          timestamp: null,
          type: 'pinterest',
          url: metatags.hasOwnProperty('og:url') ? metatags['og:url'] : ''
        }
        posts.push(p)
      }
      return posts
...
app.get('/getPosts/:keyword', async (req, res) => {
  ...
  await axios
    .all([
      getInstagramPosts(keyword),
      getTwitterPosts(keyword),
      getPinterestPosts(keyword),
      getTumblrPosts(keyword)
    ])
    .then(
